...
class Rock_Paper_Scissor:
    ...
    def createWindow(self):
        ...
        self.nameFrm = tk.Frame(self.root, bg="yellow")
        self.nameFrm.pack(pady=20)

        gameLabel=tk.Label(self.nameFrm, text="ROCK-PAPER-SCISSORS", font=("Algerian", 14), bg="yellow")
        gameLabel.grid(row=0, column=0, columnspan=2, pady=10)
        
        player1Label=tk.Label(self.nameFrm, text="Player 1 Name:", font=("Elephant", 10), bg="yellow")
        player1Label.grid(row=1, column=0, pady=10)
        self.player1data = tk.Entry(self.nameFrm)
        self.player1data.grid(row=1, column=1)

        player2Label=tk.Label(self.nameFrm, text="Player 2 Name:", font=("Elephant", 10), bg="yellow")
        player2Label.grid(row=2, column=0, pady=10)
        self.player2data = tk.Entry(self.nameFrm)
        self.player2data.grid(row=2, column=1)

        startGameBtn=tk.Button(self.nameFrm, text="Start Game", command=self.startGame, bg="orange", foreground="white")
    ...
    def setupGame(self):
        self.score_frame = tk.Frame(self.root, bg="yellow")
        self.score_frame.pack(pady=10)

        self.player1_label = tk.Label(self.score_frame, text=f"{self.player1}: 0", bg="yellow", font=("Arial", 12))
        self.player1_label.grid(row=0, column=0, padx=20)

        self.player2_label = tk.Label(self.score_frame, text=f"{self.player2}: 0", bg="yellow", font=("Arial", 12))
        self.player2_label.grid(row=0, column=1, padx=20)

        self.gameFrm = tk.Frame(self.root, bg="yellow")
        ...
        self.resultBoard = tk.Label(self.root, text="", bg="yellow", font=("Arial", 14))
        self.resultBoard.pack(pady=20)

        self.continueFrm = tk.Frame(self.root, bg="yellow")
